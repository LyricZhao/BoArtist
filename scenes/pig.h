...
double r_alpha = 0.7, sppm_radius = 10, energy = 30., camera_scale = 0.4, dof = 0, focal_distance = 1.;
...
Vector2D points[] = {
    ...
    Vector2D(38.02, 39.44), Vector2D(38.45, 37.56), Vector2D(37.79, 35.63),  Vector2D(36.87, 34.13),
    Vector2D(35.92, 33.09), Vector2D(37.08, 32.00), Vector2D(38.44, 30.54), Vector2D(40.53, 27.56),
    Vector2D(41.89, 24.59), Vector2D(42.69, 21.17), Vector2D(42.79, 18.63), Vector2D(42.54, 15.73),
    Vector2D(41.65, 12.66), Vector2D(39.65, 8.96), Vector2D(36.97, 5.87), Vector2D(33.51, 3.25),
    Vector2D(29.70, 1.46), Vector2D(26.04, 0.45), Vector2D(21.93, 0.14), Vector2D(17.99, 0.39),
    Vector2D(14.63, 1.20), Vector2D(10.64, 2.85), Vector2D(7.50, 4.89), Vector2D(4.73, 7.46),
    Vector2D(3.09, 9.53), Vector2D(1.76, 11.94), Vector2D(0.73, 14.50), Vector2D(0.16, 16.97),
    Vector2D(0.04, 19.33), Vector2D(0.46, 23.52), Vector2D(1.41, 26.39), Vector2D(3.05, 29.36),
...
Object* objects[] = {
    // new Sphere(20, Vector3D(20, 400, 20), 1.5, Texture("", Color_F()), DIFF, Color_F(1., 1., 1.)),
