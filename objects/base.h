...
public:
    ...
    double max() const { std:: max(std:: max(x, y), z); }
    double length() const { return sqrt(x * x + y * y + z * z); }
    double length2() const { return x * x + y * y + z * z; }
    double dot(const Vector3D &b) const { return x * b.x + y * b.y + z * b.z; }
    Vector3D mul(const Vector3D &b) const { return Vector3D(x * b.x, y * b.y, z * b.z); }
    Vector3D cross(const Vector3D &b) const { return Vector3D(y * b.z - z * b.y, z * b.x - x * b.z, x * b.y - y * b.x); }
    Vector3D reflect(const Vector3D &n) const { return (*this) - n * 2. * dot(n); }
